...
targetMonitor="$(aerospace list-windows --format '%{monitor-id}' --focused)"
targetApp="$(aerospace list-windows --format '%{app-name}' --focused)"
resolution="$(system_profiler -json SPDisplaysDataType | jq -r --arg monitor "$targetMonitor" '.SPDisplaysDataType[].spdisplays_ndrvs[] | select(._spdisplays_displayID == $monitor)._spdisplays_resolution')"
