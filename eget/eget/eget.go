...
func getFinder(project string, opts *Flags) (finder Finder, tool string) {
	if IsLocalFile(project) || (IsUrl(project) && !IsGithubUrl(project)) {
	  ...
	  goarch := runtime.GOARCH
		...
		project = strings.ReplaceAll(project, "{{.OS}}", goos)
		project = strings.ReplaceAll(project, "{{.Arch}}", goarch)
...
func main() {
	...
	// fmt.Fprintf(output, "Binary: %v \n", opts.Binary)
	// fmt.Fprintf(output, "Output: %v \n", opts.Output)
